class RedAlien {
  ...
  draw() {
    if (this.alive && currentActivity == 3) { 
      image(this.image, this.x, this.y, this.alienWidth, this.alienHeight);
      if(this.x < this.shotLocation && !this.redLaserFired && currentActivity == 3){
    ...
    if (!this.alive && currentActivity == 3) {
      if (this.explosionTimer > 0 && currentActivity == 3) {
        this.die();
        this.explosionTimer -= 1;
      }
    }
    if (!this.alive && this.explosionTimer == 0 && this.scoreTimer > 0 && currentActivity == 3){
      stroke(red);
      noFill();
      textSize(12);
      strokeWeight(1);
